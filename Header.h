...
void myLine(int x1, int y1, int x2, int y2, int colorR, int colorG, int colorB, int thickness) //use three 3 integers if you want colored lines.
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255) to get shades of gray. For other colors use 3 integers for colors.
	HPEN pen = CreatePen(PS_SOLID, thickness, RGB(colorR, colorG, colorB)); //2 is the width of the pen
...
void myRect(int x1, int y1, int x2, int y2, int outlineR, int outlineG, int outlineB, int fillR, int fillG, int fillB)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255)
	HPEN pen = CreatePen(PS_SOLID, 3, RGB(outlineR, outlineG, outlineB));
	SelectObject(device_context, pen);
...
void myEllipse(int x1, int y1, int x2, int y2, int outlineR, int outlineG, int outlineB, int fillR, int fillG, int fillB)
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);
	...
	HPEN pen = CreatePen(PS_SOLID, 3, RGB(outlineR, outlineG, outlineB));
	SelectObject(device_context, pen);
	HBRUSH brush = ::CreateSolidBrush(RGB(fillR, fillG, fillB));  //Fill color is black
	SelectObject(device_context, brush);
...
{
	myLine(x1, y2, x2, y2, R, G, B, 3);
	int thirdVX = (x1 + x2) / 2;
	myLine(thirdVX, y1, x1, y2, R, G, B, 3);
	myLine(thirdVX, y1, x2, y2, R, G, B, 3);
}
